...
class ColumnPosts{
	...
	{				
		...
		switch ( $options['column'] ) { 
			case 1: $cp_column_txt = "small-12 medium-6 columns"; break;
			case 3: $cp_column_txt = "small-12 medium-4 columns"; break;
			default: $cp_column_txt = "small-12";
		}
	...
	function print_posts($post, $args){
		...
		$ppost .= '<article data-layout="' . get_post_custom_values('article_layout')[0] . '" class="effect-milo">' . $thumb;
		$ppost .= '<a href="'.get_permalink($post->ID).'" title="'.sprintf(__('Article %s published at %s', 'cp'), $title_full, date_i18n(__('F j, Y g:i a'), strtotime($post->post_date)) ).'"></a>';

		$ppost .= '<div class="title"><h2>' . $title . '</h2>';

		// excerpt
		$ppost .= '<p>' .get_the_excerpt() .'</p></div></article>';
		// if ( $args['excerpt'] ) {
		// 	$ppost .= '<p>' .get_the_excerpt() .'</p>';
		// }
		// else{
		// 	$ppost .= $thclear . '</article>';
		// }
